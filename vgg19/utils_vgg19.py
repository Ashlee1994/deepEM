...
def mapstd(mrcData,dim_x,dim_y):
    ...
    # print("data:  max = %.2f, min = %.2f" % ( data.max(), data.min()))
    return data
    ...
    for num in range(start_num, end_num + 1):
        ...
        for ii in range(len(boxX)):
            ...
            for i in range(rotation_n):
                ...
                image_resize = image_rot.resize((224,224),Image.ANTIALIAS)
                image_resize = np.array(image_resize)
                image_resize = (( image_resize - image_resize.min())/(image_resize.max() - image_resize.min()) ) * 255
                # print("image_resize:  max = %.2f, min = %.2f" % ( image_resize.max(), image_resize.min()))
                particles.append(image_resize)
        mrc.close()
    return particles
...
def load_train(args):
    ...
    train_x = np.empty((train_size, args.resize, args.resize))
    train_y = np.zeros((train_size, 1))
    test_x = np.empty((test_size, args.resize, args.resize))
    ...
    train_x = train_x.reshape(len(train_x),args.resize, args.resize, 1)
    test_x = test_x.reshape(len(test_x),args.resize, args.resize, 1)
    print("train_x:  max = %.2f, min = %.2f" % ( train_x.max(), train_x.min()))
    print("test_x:  max = %.2f, min = %.2f" % ( test_x.max(), test_x.min()))
